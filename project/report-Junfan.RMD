...
randomforest <- function(training_data, testing_data, columnname="label"){
    ...
    model <- ranger(dependent.variable.name = columnname, data=training_data, classification=TRUE)
...
ktable <- function(data, nclass, nsam=494020){
    data[,1:41] <- normalize(
        data[,1:41], 
        method = "standardize", 
        range = c(0, 1), 
...
connection_events<-randomforest(sam, connection_events[,1:42], columnname = "type")
confusion_matrix <- randomforest(sam, connection_events[,1:42])
```
## Sampling 50 rows 
