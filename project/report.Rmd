...
The code below uses `rpart` for a naive implementation of a decision tree on the KDD-99 data set:
...
ggsave('./figures/decision-tree-confusion-matrix.pdf', width=6, height=5)
```
...
ggsave('./figures/weighted-decision-tree-confusion-matrix.pdf', width=6, height=5)
...
ggsave('./figures/embedded-k-means-5-confusion-matrix.pdf', width=6, height=5)
```
...
dev.copy(pdf,"./figures/embedded-k-means-5-tree.pdf", width=6, height=10)
dev.off()
```
...
Here you can see that k-means cluster is able to add information towards rare classes which is then used by the decision tree!


# Weighted Decision Tree with 2 k-means clusters classes

From results above it is clear that the vast majority of errors are from false predictions of normal trafffic for truly malicious traffic. This seems to be well distributed across all attack types. Can we use k-means with two clusters and hope that it is able to distinguish normal from non-normal traffic. Note: We planned for this to be done via k-NN so that we could be sure that we were creating clusters for normal and non-normal traffic, however, we had trouble getting the k-NN model to work in R.
...
```{r}
k_means_decision_tree_results$plot_confusion_matrix(connection_events, log=TRUE)
ggsave('./figures/embedded-k-means-2-confusion-matrix.pdf', width=6, height=5)
```
...
dev.copy(pdf,"./figures/embedded-k-means-2-tree.pdf", width=6, height=10)
dev.off()
```
...
```{r}
plot_log_confusion_matrix(summary_matrix)
ggsave('./figures/random-forest-confusion-matrix.pdf', width=6, height=5)
```
