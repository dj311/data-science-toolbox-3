...
unique(probe_connection_events$label)
```
...
probe_predictions <- predict(nb_model, test)
...
```{r eval=FALSE}
confusion_matrices <-lapply(k_folds, function(fold) {  # approx. 30 minutes
    training_indices <- -fold
    testing_indices <- fold
    ...
    confusion_matrix <- table(testing_data$label, predictions)
...
Naive bayes assumes dataset is gaussian and indepentent, so this makes us think that the data set isn't! Let's try something else.

k_means_1 adds a lot distingushing information 
